...
# Plot
rownames(arg_shannon_div)

grp <- arg_shannon_div %>%
  group_by(hospital) %>%
  mutate(average=mean(shannon))

grp

p <- ggplot(arg_shannon_div, aes(x=hospital, y=grp$average, fill = hospital)) + 
...
# Plot
rownames(arg_simpson_div)

grp <- arg_simpson_div %>%
  group_by(hospital) %>%
  mutate(average=mean(simpson))

grp

p <- ggplot(arg_simpson_div, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
#### Inverse Simpson for ARGs (box plot)
...
# Adjust the scale to be more detailed
p1 <- p1 + labs(title = "Diversity Metrics for ARGs Across Hospitals", 
                subtitle = "Inverse Simpson Diversity Index by Hospital") +
...
#### Inverse Simpson for ARGs (bar plot)

```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
arg_invsimp_div <- data.frame("invsimpson" = diversity(otu_table(resfinder_PHY_counts_HWW), index = "invsimpson", MARGIN = 2))
all(rownames(arg_invsimp_div) == colnames(resfinder_PHY_counts_HWW@otu_table))
diversity_observed <- apply(resfinder_PHY_counts_HWW@otu_table > 0, 2, sum)
arg_invsimp_div$observed <- diversity_observed
arg_invsimp_div$hospital <- as.factor(resfinder_PHY_counts_HWW@sam_data$hospital)
...
# Plot
rownames(arg_invsimp_div)

grp <- arg_invsimp_div %>%
  group_by(hospital) %>%
  mutate(average=mean(invsimpson))

grp

p <- ggplot(arg_invsimp_div, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
# Save plot
ggsave(filename = "plot5_arg_invsimp_bar.tiff",
       width = 8, height = 13, dpi = 300, units = "in", device='tiff', scale = 1)
```
...
# Get mean and standard deviation
arg_invsimp_eda <- do.call(data.frame, aggregate(richness ~ hospital, arg_rich, function(x) c(mean = mean(x), sd = sd(x))))
...
grp <- arg_rich %>%
  group_by(hospital) %>%
  mutate(average=mean(richness))

grp

p <- ggplot(arg_rich, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
grp <- arg_even %>%
  group_by(hospital) %>%
  mutate(average=mean(evenness))

grp

p <- ggplot(arg_even, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
# Adjust the scale to be more detailed
p1 <- p1 + labs(title = "Evenness for ARGs Across Hospitals", 
                subtitle = "Pieolou's Evenness by Hospital",
                y = "evenness")
...
p <- ggplot(tax_shannon_div, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
p <- ggplot(tax_simpson_div, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
#### Inverse Simpson for Taxa (metaphlan) (box plot)
...
#### Inverse Simpson for Taxa (metaphlan) (bar plot)

```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
tax_invsimp_div <- data.frame("invsimpson" = diversity(otu_table(metaphlan_PHY_counts_genus), index = "invsimpson", MARGIN = 2))
all(rownames(tax_invsimp_div) == colnames(metaphlan_PHY_counts_genus@otu_table))
diversity_observed <- apply(metaphlan_PHY_counts_genus@otu_table > 0, 2, sum)
tax_invsimp_div$observed <- diversity_observed
tax_invsimp_div$hospital <- as.factor(metaphlan_PHY_counts_genus@sam_data$hospital)
...
p <- ggplot(tax_invsimp_div, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
# Save plot
ggsave(filename = "plot6_tax_invsimp_bar.tiff",
       width = 8, height = 13, dpi = 300, units = "in", device='tiff', scale = 1)
```
...
# Get mean and standard deviation
tax_invsimp_eda <- do.call(data.frame, aggregate(richness ~ hospital, tax_rich, function(x) c(mean = mean(x), sd = sd(x))))
...
p <- ggplot(tax_rich, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
p <- ggplot(tax_even, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
p <- ggplot(mge_shannon_div, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
p <- ggplot(mge_simpson_div, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
#### Inverse Simpson for MGEs (box plot)
...
#### Inverse Simpson for MGEs (bar plot)

```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
mge_invsimp_div <- data.frame("invsimpson" = diversity(otu_table(MGE_PHY_counts_HWW), index = "invsimpson", MARGIN = 2))
all(rownames(mge_invsimp_div) == colnames(MGE_PHY_counts_HWW@otu_table))
diversity_observed <- apply(MGE_PHY_counts_HWW@otu_table > 0, 2, sum)
mge_invsimp_div$observed <- diversity_observed
mge_invsimp_div$hospital <- as.factor(MGE_PHY_counts_HWW@sam_data$hospital)
...
p <- ggplot(mge_invsimp_div, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
# Save plot
ggsave(filename = "plot7_mge_invsimp_bar.tiff",
       width = 8, height = 13, dpi = 300, units = "in", device='tiff', scale = 1)
```
...
# Get mean and standard deviation
mge_invsimp_eda <- do.call(data.frame, aggregate(richness ~ hospital, mge_rich, function(x) c(mean = mean(x), sd = sd(x))))
...
p <- ggplot(mge_rich, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
p <- ggplot(mge_even, aes(x=hospital, y=grp$average, fill = hospital)) + 
  geom_bar(stat = "identity", width = 0.7)
...
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
MGE_PHY_counts_HWW <- subset_taxa(MGE_PHY_counts_HWW, Gene != "NA")

p <- MGE_PHY_counts_HWW %>%
  comp_barplot(tax_level = "Gene",
               n_taxa = 40,
               tax_transform_for_plot = "identity",
               sample_order = sort(rownames(sam_data(MGE_PHY_counts_HWW))),
               bar_outline_colour = NA,
               bar_width = 0.7,
               other_name = "Others") +
  labs(x = NULL, y = NULL) + theme_light() + ggtitle("Counts of MGEs (Top 40)") +
  theme(text = element_text(family = "Palatino"),
        plot.title = element_text(size = 30, face = "bold"),
        axis.text.y = element_text(size = 10),
        axis.text.x = element_text(size = 20, angle = 45, hjust = 1),
...
p <- metaphlan_PHY_rel_order %>%
  comp_barplot(tax_level = "Order",
               ...
               other_name = "Others") +
  coord_flip()  +
  labs(x = NULL, y = NULL) + theme_light() + ggtitle("Relative Abundances of Top 40 Orders (metaphlan)") +
  theme(text = element_text(family = "Palatino"),
        plot.title = element_text(size = 30, face = "bold"),
        axis.text.y = element_text(size = 10),
        axis.text.x = element_text(size = 20),
...
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
metaphlan_PHY_counts_species <- subset_taxa(metaphlan_PHY_counts_species, Species != "NA")

p <- metaphlan_PHY_counts_species %>%
  comp_barplot(tax_level = "Species",
               n_taxa = 40,
               tax_transform_for_plot = "identity",
               sample_order = sort(rownames(sam_data(metaphlan_PHY_counts_species))),
               bar_outline_colour = NA,
               bar_width = 0.7,
               other_name = "Others") +
  labs(x = NULL, y = NULL) + theme_light() + ggtitle("Counts of Top 40 Species (metaphlan)") +
  theme(text = element_text(family = "Palatino"),
        plot.title = element_text(size = 30, face = "bold"),
        axis.text.y = element_text(size = 10),
        axis.text.x = element_text(size = 20, angle = 45, hjust = 1),
...
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
metaphlan_PHY_counts_genus <- subset_taxa(metaphlan_PHY_counts_genus, Genus != "Coprobacillaceae Family")
metaphlan_PHY_counts_genus <- subset_taxa(metaphlan_PHY_counts_genus, Genus != "Catenibacterium")
metaphlan_PHY_counts_genus <- subset_taxa(metaphlan_PHY_counts_genus, Genus != "NA")

p <- metaphlan_PHY_counts_genus %>%
  comp_barplot(tax_level = "Genus",
               n_taxa = 40,
               tax_transform_for_plot = "identity",
               sample_order = sort(rownames(sam_data(metaphlan_PHY_counts_genus))),
               bar_outline_colour = NA,
               bar_width = 0.7,
               other_name = "Others") +
  labs(x = NULL, y = NULL) + theme_light() + ggtitle("Counts of Top 40 Genera (metaphlan)") +
  theme(text = element_text(family = "Palatino"),
        plot.title = element_text(size = 30, face = "bold"),
        axis.text.y = element_text(size = 10),
        axis.text.x = element_text(size = 20, angle = 45, hjust = 1),
...
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
metaphlan_PHY_counts_family <- subset_taxa(metaphlan_PHY_counts_family, Family != "Coprobacillaceae")
metaphlan_PHY_counts_family <- subset_taxa(metaphlan_PHY_counts_family, Family != "NA")

p <- metaphlan_PHY_counts_family %>%
  comp_barplot(tax_level = "Family",
               n_taxa = 40,
               tax_transform_for_plot = "identity",
               sample_order = sort(rownames(sam_data(metaphlan_PHY_counts_family))),
               bar_outline_colour = NA,
               bar_width = 0.7,
               other_name = "Others") +
  labs(x = NULL, y = NULL) + theme_light() + ggtitle("Counts of Top 40 Families (metaphlan)") +
  theme(text = element_text(family = "Palatino"),
        plot.title = element_text(size = 30, face = "bold"),
        axis.text.y = element_text(size = 10),
        axis.text.x = element_text(size = 20, angle = 45, hjust = 1),
...
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
metaphlan_PHY_counts_order <- subset_taxa(metaphlan_PHY_counts_order, Order != "Eubacteriales")
metaphlan_PHY_counts_order <- subset_taxa(metaphlan_PHY_counts_order, Order != "Erysipelotrichales")
metaphlan_PHY_counts_order <- subset_taxa(metaphlan_PHY_counts_order, Order != "NA")

p <- metaphlan_PHY_counts_order %>%
  comp_barplot(tax_level = "Order",
               n_taxa = 40,
               tax_transform_for_plot = "identity",
               sample_order = sort(rownames(sam_data(metaphlan_PHY_counts_order))),
               bar_outline_colour = NA,
               bar_width = 0.7,
               other_name = "Others") +
  labs(x = NULL, y = NULL) + theme_light() + ggtitle("Counts of Top 40 Orders (metaphlan)") +
  theme(text = element_text(family = "Palatino"),
        plot.title = element_text(size = 30, face = "bold"),
        axis.text.y = element_text(size = 10),
        axis.text.x = element_text(size = 20, angle = 45, hjust = 1),
...
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
metaphlan_PHY_counts_class <- subset_taxa(metaphlan_PHY_counts_class, Class != "Clostridia")
metaphlan_PHY_counts_class <- subset_taxa(metaphlan_PHY_counts_class, Class != "Erysipelotrichia")
metaphlan_PHY_counts_class <- subset_taxa(metaphlan_PHY_counts_class, Class != "NA")

p <- metaphlan_PHY_counts_class %>%
  comp_barplot(tax_level = "Class",
               n_taxa = 40,
               tax_transform_for_plot = "identity",
               sample_order = sort(rownames(sam_data(metaphlan_PHY_counts_class))),
               bar_outline_colour = NA,
               bar_width = 0.7,
               other_name = "Others") +
  labs(x = NULL, y = NULL) + theme_light() + ggtitle("Counts of Top 40 Classes (metaphlan)") +
  theme(text = element_text(family = "Palatino"),
        plot.title = element_text(size = 30, face = "bold"),
        axis.text.y = element_text(size = 10),
        axis.text.x = element_text(size = 20, angle = 45, hjust = 1),
...
```{r, warning=FALSE, error=FALSE, message=FALSE, results='hide', fig.show='hide'}
metaphlan_PHY_counts_phylum <- subset_taxa(metaphlan_PHY_counts_phylum, Phylum != "NA")

p <- metaphlan_PHY_counts_phylum %>%
  comp_barplot(tax_level = "Phylum",
               n_taxa = 40,
               tax_transform_for_plot = "identity",
               sample_order = sort(rownames(sam_data(metaphlan_PHY_counts_phylum))),
               bar_outline_colour = NA,
               bar_width = 0.7,
               other_name = "Others") +
  labs(x = NULL, y = NULL) + theme_light() + ggtitle("Counts of Phyla (metaphlan)") +
  theme(text = element_text(family = "Palatino"),
        plot.title = element_text(size = 30, face = "bold"),
        axis.text.y = element_text(size = 10),
        axis.text.x = element_text(size = 20, angle = 45, hjust = 1),
...
```{r}
write_csv(psmelt(resfinder_PHY_counts_HWW), file = 'resfinder_PHY_counts_HWW.csv')
write_csv(psmelt(resfinder_PHY_rel_HWW), file = 'resfinder_PHY_rel_HWW.csv')
write_csv(psmelt(MGE_PHY_counts_HWW), file = 'MGE_PHY_counts_HWW.csv')
write_csv(psmelt(MGE_rel_PHY), file = 'MGE_PHY_rel_HWW.csv')
write_csv(psmelt(metaphlan_PHY_counts_phylum), file = 'metaphlan_PHY_counts_phylum.csv')
write_csv(psmelt(metaphlan_PHY_counts_class), file = 'metaphlan_PHY_counts_class.csv')
write_csv(psmelt(metaphlan_PHY_counts_order), file = 'metaphlan_PHY_counts_order.csv')
write_csv(psmelt(metaphlan_PHY_counts_family), file = 'metaphlan_PHY_counts_family.csv')
write_csv(psmelt(metaphlan_PHY_counts_genus), file = 'metaphlan_PHY_counts_genus.csv')
write_csv(psmelt(metaphlan_PHY_counts_species), file = 'metaphlan_PHY_counts_species.csv')
