...
all(tax_table_metaphlan$V1 == mp_rel$clade_name)
...
a2_plot + plot_annotation(tag_levels = list(c("A")), title = "PCA Plot for three hospitals on Resistomes", subtitle = "ARGs") & 
...
resfinder_PHY_counts_uniq_temp <- subset_samples(resfinder_PHY_counts_uniq, (hospital == "A" | hospital == "C"))
...
```{r}
# Test the significance 
resfinder_PHY_counts_uniq_temp <- subset_samples(resfinder_PHY_counts_uniq, (hospital == "A" | hospital == "B"))
resfinder_PHY_counts_uniq_temp %>%
  dist_calc("aitchison") %>%
  dist_permanova(variables = c("hospital"), n_perms = 9999, seed = 12345)
```
...
```{r}
# Test the significance 
resfinder_PHY_counts_uniq_temp <- subset_samples(resfinder_PHY_counts_uniq, (hospital == "B" | hospital == "C"))
resfinder_PHY_counts_uniq_temp %>%
  dist_calc("aitchison") %>%
  dist_permanova(variables = c("hospital"), n_perms = 9999, seed = 12345)
```
...
b2_plot + plot_annotation(tag_levels = list(c("A")), title = "PCA Plot for three hospitals on Taxonomic Composition", subtitle = "Metaphlan") & 
  theme(plot.title = element_text(size = 25, family = "Palatino", face = "bold"),
        plot.subtitle = element_text(size = 25, family = "Palatino", hjust = 0.5),
    plot.tag = element_text(size = 25))
...
# Save plots for all samples
c2+ plot_annotation(tag_levels = list(c("C")), title = "PCA Plot with clr-transformed data on Mobilomes", subtitle = "MGEs") & 
  theme(plot.title = element_text(size = 25, family = "Palatino", face = "bold"),
        plot.subtitle = element_text(size = 25, family = "Palatino", hjust = 0.5),
    plot.tag = element_text(size = 25))
...
div$"hospital" <- cbind(resfinder_PHY_counts_HWW@sam_data$hospital)
...
p1 <- p1 + labs(title = "Diversity Metrics for ARGs Across Hospitals", 
  ...
  scale_y_continuous(breaks = seq(0, max(div$shannon), by = 0.5))
...
p <- ggboxplot(df, x = "hospital", y = "RA", color = "hospital", palette = c("#1C4731", "#910C28", "#1E88E5"), width = 0.3)  +
  ...
  geom_text(data = df, aes(x = hospital, y = RA, label = rownames(div)), size = 3, vjust = -0.5)  +
