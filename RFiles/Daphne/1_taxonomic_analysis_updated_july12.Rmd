...
metaphlan_data <- read.delim("merged_metaphlan_rel_species.txt", header = TRUE)
...
```{r}
selected_data <- wide_phylum_data%>% select(c(18: ncol(wide_phylum_data)))
selected_data <- clr(selected_data)
pca_result <- rda(selected_data)
...
# Add the hospital column to the PCA scores data frame
pca_data$hospital <- wide_phylum_data$hospital
pca_data$SampleID <- wide_phylum_data$SampleID
pca_data$month <- wide_phylum_data$month
...
p + ggtitle("PCA, clr-transformed for three hospitals on Taxonomic Composition at phylum Level", subtitle = "Metaphlan")

# Save the plot
ggsave(filename = "plot1_tax_pca_phylum.png",
       width = 16, height = 13, dpi = 300, units = "in", device = 'png', scale = 1)
```

### Relative Abundance Separated Per Hospital

#### CGH Species

```{r}
# Filter data for the CGH hospital
cgh_species_data <- species_data %>% 
  filter(hospital == "CGH")

# Summarize and get the top species for CGH hospital
top_species <- cgh_species_data %>%
  group_by(clade_name) %>%
  summarise(AverageAbundance = mean(RelativeAbundance, na.rm = TRUE)) %>%
  arrange(desc(AverageAbundance)) %>%
  top_n(15, AverageAbundance) %>%
...
# Update species_viz to only include data for CGH hospital
species_viz <- cgh_species_data %>%
  mutate(clade_name = ifelse(clade_name %in% top_species, clade_name, "Others"))
...
# Create the vertical bar plot using ggplot2
p <- ggplot(species_viz, aes(x = SampleID, y = RelativeAbundance, fill = clade_name)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  color_palette +
  labs(x = "Sample", y = "Abundance %", title = "Species Relative Abundances for CGH Hospital") +
  theme_light() +
  theme(legend.position = "right", 
        legend.title = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1),
...
# Save plot
ggsave(filename = "plot12_tax_rel_species_cgh.png",
       width = 16, height = 8, dpi = 300, units = "in", device = 'png', scale = 1)
...
slmc_species_data <- species_data %>% 
  filter(hospital == "SLMC")

# Summarize and get the top species for SLMC hospital
top_species <- slmc_species_data %>%
  group_by(clade_name) %>%
  summarise(AverageAbundance = mean(RelativeAbundance, na.rm = TRUE)) %>%
  arrange(desc(AverageAbundance)) %>%
  top_n(15, AverageAbundance) %>%
...
# Update species_viz to only include data for SLMC hospital
species_viz <- slmc_species_data %>%
  mutate(clade_name = ifelse(clade_name %in% top_species, clade_name, "Others"))
...
p <- ggplot(species_viz, aes(x = SampleID, y = RelativeAbundance, fill = clade_name)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  color_palette +
  labs(x = "Sample", y = "Abundance %", title = "Species Relative Abundances for SLMC Hospital") +
  theme_light() +
  theme(legend.position = "right", 
        legend.title = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1),
...
# Save plot
ggsave(filename = "plot12_tax_rel_species_slmc.png",
       width = 16, height = 8, dpi = 300, units = "in", device = 'png', scale = 1)
...
ust_species_data <- species_data %>% 
  filter(hospital == "UST")

# Summarize and get the top species for UST hospital
top_species <- ust_species_data %>%
  group_by(clade_name) %>%
  summarise(AverageAbundance = mean(RelativeAbundance, na.rm = TRUE)) %>%
  arrange(desc(AverageAbundance)) %>%
  top_n(15, AverageAbundance) %>%
...
# Update species_viz to only include data for UST hospital
species_viz <- ust_species_data %>%
  mutate(clade_name = ifelse(clade_name %in% top_species, clade_name, "Others"))
...
p <- ggplot(species_viz, aes(x = SampleID, y = RelativeAbundance, fill = clade_name)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  color_palette +
  labs(x = "Sample", y = "Abundance %", title = "Species Relative Abundances for UST Hospital") +
  theme_light() +
  theme(legend.position = "right", 
        legend.title = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1),
...
# Save plot
ggsave(filename = "plot12_tax_rel_species_ust.png",
       width = 16, height = 8, dpi = 300, units = "in", device = 'png', scale = 1)
...
```{r}
# Filter data for the CGH hospital
cgh_genus_data <- genus_data %>% 
  filter(hospital == "CGH")

# Summarize and get the top genus for CGH hospital
top_genus <- cgh_genus_data %>%
  group_by(clade_name) %>%
  summarise(AverageAbundance = mean(RelativeAbundance, na.rm = TRUE)) %>%
  arrange(desc(AverageAbundance)) %>%
  top_n(15, AverageAbundance) %>%
...
# Update genus_viz to only include data for CGH hospital
genus_viz <- cgh_genus_data %>%
  mutate(clade_name = ifelse(clade_name %in% top_genus, clade_name, "Others"))
...
p <- ggplot(genus_viz, aes(x = SampleID, y = RelativeAbundance, fill = clade_name)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  color_palette +
  labs(x = "Sample", y = "Abundance %", title = "Genus Relative Abundances for CGH Hospital") +
  theme_light() +
  theme(legend.position = "right", 
        legend.title = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1),
...
# Save plot
ggsave(filename = "plot12_tax_rel_genus_cgh.png",
       width = 16, height = 8, dpi = 300, units = "in", device = 'png', scale = 1)
...
# Filter data for the SLMC hospital
slmc_genus_data <- genus_data %>% 
  filter(hospital == "SLMC")

# Summarize and get the top genus for SLMC hospital
top_genus <- slmc_genus_data %>%
  group_by(clade_name) %>%
  summarise(AverageAbundance = mean(RelativeAbundance, na.rm = TRUE)) %>%
  arrange(desc(AverageAbundance)) %>%
  top_n(15, AverageAbundance) %>%
...
# Update genus_viz to only include data for SLMC hospital
genus_viz <- slmc_genus_data %>%
  mutate(clade_name = ifelse(clade_name %in% top_genus, clade_name, "Others"))
...
p <- ggplot(genus_viz, aes(x = SampleID, y = RelativeAbundance, fill = clade_name)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  color_palette +
  labs(x = "Sample", y = "Abundance %", title = "Genus Relative Abundances for SLMC Hospital") +
  theme_light() +
  theme(legend.position = "right", 
        legend.title = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1),
...
# Save plot
ggsave(filename = "plot12_tax_rel_genus_slmc.png",
       width = 16, height = 8, dpi = 300, units = "in", device = 'png', scale = 1)
```
...
```{r}
# Filter data for the UST hospital
ust_genus_data <- genus_data %>% 
  filter(hospital == "UST")

# Summarize and get the top genus for UST hospital
top_genus <- ust_genus_data %>%
  group_by(clade_name) %>%
  summarise(AverageAbundance = mean(RelativeAbundance, na.rm = TRUE)) %>%
  arrange(desc(AverageAbundance)) %>%
  top_n(15, AverageAbundance) %>%
...
# Update genus_viz to only include data for UST hospital
genus_viz <- ust_genus_data %>%
  mutate(clade_name = ifelse(clade_name %in% top_genus, clade_name, "Others"))
...
p <- ggplot(genus_viz, aes(x = SampleID, y = RelativeAbundance, fill = clade_name)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  color_palette +
  labs(x = "Sample", y = "Abundance %", title = "Genus Relative Abundances for UST Hospital") +
  theme_light() +
  theme(legend.position = "right", 
        legend.title = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1),
...
# Save plot
ggsave(filename = "plot12_tax_rel_genus_ust.png",
       width = 16, height = 8, dpi = 300, units = "in", device = 'png', scale = 1)
```
...
```{r}
# Filter data for the CGH hospital
cgh_phylum_data <- phylum_data %>% 
  filter(hospital == "CGH")

# Summarize and get the top phylum for CGH hospital
top_phylum <- cgh_phylum_data %>%
  group_by(clade_name) %>%
  summarise(AverageAbundance = mean(RelativeAbundance, na.rm = TRUE)) %>%
  arrange(desc(AverageAbundance)) %>%
  top_n(15, AverageAbundance) %>%
...
# Update phylum_viz to only include data for CGH hospital
phylum_viz <- cgh_phylum_data %>%
  mutate(clade_name = ifelse(clade_name %in% top_phylum, clade_name, "Others"))
...
p <- ggplot(phylum_viz, aes(x = SampleID, y = RelativeAbundance, fill = clade_name)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  color_palette +
  labs(x = "Sample", y = "Abundance %", title = "Phylum Relative Abundances for CGH Hospital") +
  theme_light() +
  theme(legend.position = "right", 
        legend.title = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1),
...
# Save plot
ggsave(filename = "plot12_tax_rel_phylum_cgh.png",
       width = 16, height = 8, dpi = 300, units = "in", device = 'png', scale = 1)
```
...
```{r}
# Filter data for the SLMC hospital
slmc_phylum_data <- phylum_data %>% 
  filter(hospital == "SLMC")

# Summarize and get the top phylum for SLMC hospital
top_phylum <- slmc_phylum_data %>%
  group_by(clade_name) %>%
  summarise(AverageAbundance = mean(RelativeAbundance, na.rm = TRUE)) %>%
  arrange(desc(AverageAbundance)) %>%
  top_n(15, AverageAbundance) %>%
...
# Update phylum_viz to only include data for SLMC hospital
phylum_viz <- slmc_phylum_data %>%
  mutate(clade_name = ifelse(clade_name %in% top_phylum, clade_name, "Others"))
...
p <- ggplot(phylum_viz, aes(x = SampleID, y = RelativeAbundance, fill = clade_name)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  color_palette +
  labs(x = "Sample", y = "Abundance %", title = "Phylum Relative Abundances for SLMC Hospital") +
  theme_light() +
  theme(legend.position = "right", 
        legend.title = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1),
...
# Save plot
ggsave(filename = "plot12_tax_rel_phylum_slmc.png",
       width = 16, height = 8, dpi = 300, units = "in", device = 'png', scale = 1)
```
...
```{r}
# Filter data for the UST hospital
ust_phylum_data <- phylum_data %>% 
  filter(hospital == "UST")

# Summarize and get the top phylum for UST hospital
top_phylum <- ust_phylum_data %>%
  group_by(clade_name) %>%
  summarise(AverageAbundance = mean(RelativeAbundance, na.rm = TRUE)) %>%
  arrange(desc(AverageAbundance)) %>%
  top_n(15, AverageAbundance) %>%
...
# Update phylum_viz to only include data for UST hospital
phylum_viz <- ust_phylum_data %>%
  mutate(clade_name = ifelse(clade_name %in% top_phylum, clade_name, "Others"))
...
p <- ggplot(phylum_viz, aes(x = SampleID, y = RelativeAbundance, fill = clade_name)) +
  geom_bar(stat = "identity", position = "stack", width = 0.7) +
  color_palette +
  labs(x = "Sample", y = "Abundance %", title = "Phylum Relative Abundances for UST Hospital") +
  theme_light() +
  theme(legend.position = "right", 
        legend.title = element_blank(), 
        axis.text.x = element_text(angle = 90, hjust = 1),
...
# Save plot
ggsave(filename = "plot12_tax_rel_phylum_ust.png",
       width = 16, height = 8, dpi = 300, units = "in", device = 'png', scale = 1)
```
